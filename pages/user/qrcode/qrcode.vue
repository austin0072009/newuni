			...
			<view class="template-list">
				...
				<!-- #ifdef H5 -->
					<view class="tips text-sm wanl-gray">
						<text class="wlIcon-31tishi"></text> {{$t('user.qrcode_msg1')}}
					</view>
				<!-- #endif -->
				<!-- #ifndef H5 -->
					<view class="tips text-sm wanl-orange" @tap="save()">
						<text class="wlIcon-image margin-right-xs"></text>{{$t('user.qrcode_msg2')}}
					</view>
				<!-- #endif -->
			</view>
		</view>
...
export default {
	...
	onLoad(option) {
		uni.setNavigationBarTitle({
			title:this.$t('user.qrcode_title')
		})
		// 提示
		uni.showLoading({
			title: this.$t('user.qrcode_msg3'),
			mask: true
		});
		if(option.url){
				...
				success: res => {
					...
					if(res.length == 0){
						uni.showModal({
						    ...
						    content: this.$t('user.qrcode_msg5'),
						    success: (r)=> {
