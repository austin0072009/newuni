<template>
	<view class="wanl-rate-class wanl-rate-box" @touchmove="touchMove">
		<block v-for="(item,index) in quantity" :key="index">
			<view class="wanl-icon" :class="['wanl-icon-collection'+(hollow && current <= index?'':'-fill')]" :data-index="index"
			 @tap="handleTap" :style="{fontSize:size+'px',color:current-1 >= index?active:normal}"></view>
		</block>
	</view>
</template>
...
<style>
	...
	.wanl-icon-collection-fill:before {
		content: "\e6ea";
	}

	.wanl-icon-collection:before {
		content: "\e6eb";
	}
