		...
		<view class="wanl-order"> 
				...
				<view class="cu-item">
					...
					<view class="content">
						...
						<view class="wanl-pip text-cut-2 text-sm">
							...
							<text class="margin-left-xs" v-if="orderData.address.address_name">（{{orderData.address.address_name}}）{{$t('order.details_msg1')}}</text>
						</view>
					</view>
				</view>
			</view>
			<view class="lists bg-white"  v-if="orderData.order_no">
					...
					<block v-for="(item, index) in orderData.goods" :key="item.id">
						<view class="cu-item">
							...
							<view class="action text-sm">
								...
								<view class="text-orange">{{$t('order.details_msg2')}} <text class="text-price">{{item.actual_payment}}</text></view>
							</view>
						</view>
				...
				<view class="price padding-lr-bj padding-bottom-bj text-sm">
					<view> <text> {{$t('order.details_msg3')}} </text> <text class="text-price"> {{orderData.pay.order_price}} </text> </view>
					<view> <text> {{$t('order.details_msg4')}} </text> <text class="text-price"> {{orderData.pay.freight_price}} </text> </view>
					<view> <text> {{$t('order.preferential')}}</text> <text class="text-price"> {{orderData.pay.discount_price}} </text> </view>
					<view> <text> {{$t('order.details_msg5')}} </text> <text class="text-price"> {{orderData.pay.price}} </text> </view>
					<view class="text-sm" v-if="orderData.pay.actual_payment != 0"> <text> {{$t('order.real_payment')}} </text> <text class="text-price wanl-orange"> {{orderData.pay.actual_payment}} </text> </view>
				</view>
			</view>
			<view class="order bg-white margin-top-bj padding-bj" v-if="orderData.order_no">
				<view class="title" @tap="toMore">
					<view class="text-sm">{{$t('notice.chat_msg1')}}</view>
					<view>
				...
				<view class="text-sm">
					<view class="item">
						<text class="wanl-gray" style="width: 160rpx">{{$t('order.details_msg6')}}：</text>
						<text> {{orderData.order_no}} </text>
					</view>
					<view class="item">
						<text class="wanl-gray" style="width: 160rpx">{{$t('order.details_msg7')}}：</text>
						<text> {{orderData.pay.pay_no}} </text>
					</view>
					<view class="item">
						<text class="wanl-gray" style="width: 160rpx">{{$t('money.creation_time')}}：</text>
						<text> {{orderData.createtime_text}} </text>
					</view>
					<block v-if="infoMore">
						<view class="item" v-if="orderData.paymenttime">
							<text class="wanl-gray-dark" style="width: 160rpx">{{$t('order.details_msg8')}}：</text>
							<text> {{orderData.paymenttime_text}} </text>
						</view>
						<view class="item" v-if="orderData.delivertime">
							<text class="wanl-gray-dark" style="width: 160rpx">{{$t('order.details_msg9')}}：</text>
							<text> {{orderData.delivertime_text}} </text>
						</view>
						<view class="item" v-if="orderData.taketime">
							<text class="wanl-gray-dark" style="width: 160rpx">{{$t('order.details_msg10')}}：</text>
				...
				<view class="foot solid-top" >
					<view @tap="toChat(orderData.shop_id)">
						<text class="wlIcon-xiaoxizhongxin text-sm"></text>{{$t('order.details_msg11')}}
					</view>
				</view>
			</view>
...
export default {
	...
	onLoad(option) {
		uni.setNavigationBarTitle({
			title:this.$t('order.details_title')
		})
	...
	methods: {
		...
		async loadData(data) {
			this.$api.get({
				...
				success: res => {
					...
					if(res.state == 0){
						this.orderData.statetime = this.$t('order.details_msg12') + '0';
					}else if(res.state == 1){
						...
						if (minutes > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +minutes+ this.$t('order.details_msg14');
						}
						if (hours > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +hours+ this.$t('signin.day') +minutes+ this.$t('order.details_msg14');
						}
						if (days > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +days+ this.$t('signin.day') +hours+ this.$t('order.details_msg19') +minutes+ this.$t('order.details_msg14');
						}
					}else if(res.state == 2){
						this.orderData.statetime = this.$t('order.details_msg15');
					}else if(res.state == 3){
						...
						var minutes = Math.floor(diff % 86400 % 3600 / 60);
						if (minutes > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +minutes+ this.$t('order.details_msg16');
						}
						if (hours > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +hours+ this.$t('signin.day') +minutes+ this.$t('order.details_msg16');
						}
						if (days > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +days+ this.$t('signin.day') +hours+ this.$t('order.details_msg19') +minutes+ this.$t('order.details_msg16');
						}
					}else if(res.state == 4){
						...
						var minutes = Math.floor(diff % 86400 % 3600 / 60);
						if (minutes > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +minutes+ this.$t('order.details_msg17');
						}
						if (hours > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +hours+ this.$t('signin.day') +minutes+ this.$t('order.details_msg17');
						}
						if (days > 0) {
							this.orderData.statetime = this.$t('order.details_msg13') +days+ this.$t('signin.day') +hours+ this.$t('order.details_msg19') +minutes+ this.$t('order.details_msg17');
						}
					}else if(res.state == 6){	
						this.orderData.statetime = res.dealtime_text;
					}else if(res.state == 7){	
						this.orderData.statetime = this.$t('order.details_msg18');
					}
				}
			});
		},
		...
		getStatusText(status) {
			return [this.$t('notice.chat_msg11'), this.$t('notice.chat_msg12'), this.$t('user.pending_receipt'), this.$t('user.to_be_commented'), this.$t('notice.chat_msg13'), this.$t('notice.chat_msg14'), this.$t('notice.chat_msg15')][status - 1];
		},
		getRefund(status) {
