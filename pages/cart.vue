<template>
	<view>
				...
				<view class="wanl-cart-goods" v-for="(goods, keys) in item.products" :key="keys">
					...
					<view class="content">
						...
						<view class="cu-tag wanl-gray opt">
							{{$t('cart.specification')}}：{{ goods.sku.difference.join(' ') }}
						</view>
		...
		<block class="margin-bj padding-bj" v-else>
			<wanl-empty src="cart_default3x" :text="$t('cart.empty')" />
		...
		<view class="wanl-cart-foot fixedView solid-top edit" v-if="cart.operate">
			<view class="flex align-center" @tap="toCartchoose()">
				...
				<view class="margin-left-sm">
					<text>{{ cart.status ? $t('cart.cancel') : $t('cart.select_all') }}</text>
				</view>
			</view>
			<view class="flex">
				<button v-if="cart.allnum == 0" class="cu-btn round line-gray">{{$t('cart.mobile_focus')}}</button>
				<button v-else class="cu-btn round line-orange" @tap="move()">{{$t('cart.mobile_focus')}}</button>
				<button class="cu-btn round line-orange" @tap="toEmpty()">{{$t('cart.quick_clean')}}</button>
				<button v-if="cart.allnum == 0" class="cu-btn round line-gray">{{$t('cart.delete')}}</button>
				<button v-else class="cu-btn round bg-gradual-orange" @tap="del()">{{$t('cart.delete')}}</button>
			</view>
		</view>
		<view class="wanl-cart-foot fixedView solid-top account" v-else>
			...
			<view class="flex">
				<view class="text-sm text-right">
					...
					<view>{{$t('cart.freight_not_included')}}</view>
				</view>
				<button v-if="cart.allnum == 0" class="cu-btn round line-gray">{{$t('cart.go_to_settlement')}}</button>
				<button v-else class="cu-btn round bg-gradual-orange" @tap="settlement()">
					{{$t('cart.go_to_settlement')}} ({{ cart.allnum }})
				</button>
			</view>
		</view>
	</view>
...
export default {
	...
	methods: {
		...
		toEmpty() {
			uni.showModal({
				content: this.$t('cart.delete_cart'),
				confirmText: this.$t('cart.determine'),
				cancelText:this.$t('cart.cancel'),
				success: res => {
					if (res.confirm) {
