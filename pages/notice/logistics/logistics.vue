			...
			<view class="item margin-lr-bj margin-tb bg-white radius-bock padding-bj" @tap="$wanlshop.to(item.url)">
				...
				<view class="action">
					...
					<view class="padding-xs flex flex-wrap align-content-between">
						...
						<view class="text-sm wanl-gray">{{$t('notice.msg8')}} {{item.come}}</view>
					</view>
				</view>
			</view>
			</block>
		</view>
		<wanl-empty src="notice_default3x" :text="$t('notice.logistics_empty')" v-else/>
	...
	export default {
		...
		onLoad() {
			uni.setNavigationBarTitle({
				title:this.$t('notice.logistics_title')
			})
			this.loadData();
