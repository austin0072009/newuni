<template>
	<view class="wanl-money">
		...
		<view v-if="dataList.length == 0">
			<wanl-empty src="ticket_default3x" :text="$t('money.list_empty')"/>
		</view>
...
export default {
	data() {
		return {
			dataList: [],
			payType: {
				pay: this.$t('money.commodity_trading'),
				groups: this.$t('index.spell_group'),
				recharge: this.$t('money.top_up'),
				withdraw: this.$t('money.withdrawal'),
				refund: this.$t('order.refund'),
				sys: this.$t('money.system'),
			},
			reload: false, //判断是否上拉
			total: 0, //数据量
			current_page: 1, //当前页码
