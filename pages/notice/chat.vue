			 ...
			 :scroll-into-view="scrollToView" @scrolltoupper="loadHistory" upper-threshold="50">
				...
				<view v-for="(row, index) in msgList" :key="index" :id="'msg' + row.id">
					...
					<view v-if="row.type == 'sys'">
						<!-- 系统消息 -->
						<view class="cu-info round">{{$t('notice.chat_msg2')}}</view>
					</view>
				</view>
			</scroll-view>
		</view>
		...
		<view class="chatTips bg-white radius-bock margin-lr-bj padding-sm" v-if="goods && isGoods">
			...
			<view class="details">
				...
				<view class="flex justify-between">
					...
					<view>
						<button class="cu-btn round bg-orange sm" @tap="sendGoods(goods)">{{$t('notice.chat_msg6')}}</button>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="WANL-MODAL">
				...
				<view class="cu-dialog bg-white" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">{{$t('notice.chat_msg7')}}</view></view>
								...
								<block v-if="item.goods !== null">
									...
									<view class="details">
										...
										<view class="flex justify-between info">
											...
											<button class="cu-btn round line-orange sm" @tap="sendGoods(item.goods)">{{$t('notice.chat_msg8')}}</button>
										</view>
									</view>
								</block>
							</view>
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'order' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog bg-bgcolor" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">{{$t('notice.chat_msg9')}}</view></view>
							<view class="close wlIcon-31guanbi" @tap="hideModal"></view>
						</view>
						<scroll-view class="scroll-y" scroll-y="true">
							<view class="item bg-white radius-bock margin-tb-sm padding-bj" v-for="(item, index) in orderData" :key="item.id" @tap="sendOrder(item)">
								<view class="flex justify-between">
									<text>{{$t('money.order_no')}}：{{item.order_no}}</text>
								...
								<view class="flex justify-end">
									<button class="cu-btn round sm line-orange">{{$t('notice.chat_msg10')}}</button>
								</view>
							</view>
						</scroll-view>
						<view class="foot padding-lr-bj"><button class="cu-btn bg-gradual-orange round text-bold complete" @tap="hideModal">{{$t('gg.msg1')}}</button></view>
...
export default {
	data() {
		return {
			...
			// #endif
			isVoice: false,
			voiceTis: this.$t('service.msg8'),
			recordTis: this.$t('service.msg9'),
			recording: false,
			willStop: false,
			...
			hideMore: true,
			modalName: null,
			stateText: [this.$t('notice.chat_msg11'), this.$t('notice.chat_msg12'), this.$t('user.pending_receipt'), this.$t('user.to_be_commented'), this.$t('notice.chat_msg13'), this.$t('notice.chat_msg14'), this.$t('notice.chat_msg15')],
			refundStatusText: [this.$t('notice.chat_msg16'), this.$t('order.refund_of'), this.$t('order.for_a_refund'), this.$t('order.refund_to_complete'), this.$t('order.refund_shut_down'), this.$t('order.refund_was_rejected')],
			refundStatusBg: ["", "bg-orange", "bg-red", "bg-green", "bg-grey", "bg-red"],
			//表情定义
			//表情
			TabCur: this.$t('service.msg10'),
			hideEmoji: true,
	...
	onLoad(option) {
		...
		this.$api.post({
			...
			success: res => {
				...
				this.$wanlshop.title(res.shopname + (res.isOnline == 1 ? this.$t('notice.chat_msg17') : this.$t('notice.chat_msg18')));
				// 初始界面
	...
	methods: {
		...
		emojiData() {
			...
			emotions.forEach(emotion => {
				var cate = emotion.category.length > 0 ? emotion.category : this.$t('service.msg10');
				if (!groups[cate]) {
					groups[cate] = [];
		...
		recordBegin(e) {
			this.recording = true;
			this.voiceTis = this.$t('service.msg11');
			this.recordLength = 0;
		...
		voiceCancel() {
			...
			this.voiceTis = this.$t('service.msg8');
			this.recordTis = this.$t('service.msg9');
		...
		voiceIng(e) {
			...
			if (this.initPoint.Y - touche.clientY >= uni.upx2px(200)) {
				this.willStop = true;
				this.recordTis = this.$t('service.msg12');
			} else {
				this.willStop = false;
				this.recordTis = this.$t('service.msg9');
			}
