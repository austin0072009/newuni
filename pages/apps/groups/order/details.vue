	...
	<view>
		...
		<view class="wanl-order"> 
			...
			<view class="address cu-list menu-avatar">
				...
				<view class="cu-item" @tap="onLogistics(orderData.id)" v-if="orderData.state != 7 && orderData.state != 1 && orderData.state != 2 && orderData.state != 3 && logisticsType">
				...
				<view class="cu-list menu-avatar">
					<block v-for="(item, index) in orderData.goods" :key="item.id">
						...
						<view class="flex justify-end padding-lr-bj padding-bottom">
							<button class="cu-btn line-black sm" @tap="onRefund(orderData.id, item.refund_status, item.refund_id, index)" v-if="([2,3,4,5,6].indexOf(orderData.state)) !== -1"> {{getRefund(item.refund_status)}}</button>
							<!-- 拼团详情 -->
							<button
								v-if="orderData.state > 1 && orderData.state < 7 && item.group_type !== 'alone'"
								class="cu-btn bg-gradual-orange sm margin-left-sm" 
								@tap="onGroups(item.group_no)"
							> 
								拼团详情
							</button>
						</view>
