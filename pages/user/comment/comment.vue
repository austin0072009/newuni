<template>
	<view>
		...
		<view class="wanl-goods-comment">
			<wanl-empty :text="$t('comment.empty')" v-if="listData.length === 0 && status !== 'loading'"></wanl-empty>
			<view class="list padding-bj radius-bock" v-for="item in listData" :key="item.id">
				...
				<view class="details">
					<view class="margin-bottom-sm">{{ item.content || $t('comment.no_evaluation_content')}}</view>
				</view>
