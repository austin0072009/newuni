	...
	<view class="wanl-comment">
		...
		<view class="subject margin-lr-bj bg-white radius-bock margin-bottom-bj" v-for="(item, index) in goodsList" :key="item.id">
			...
			<view class="comment padding-lr-bj padding-top-bj">
				<view class="comment-title">{{$t('order.comment_msg1')}}</view>
			...
			<view class="describe cu-form-group margin-bj radius-bock padding-bj">
				<textarea maxlength="-1" v-model="item.comment" :placeholder="$t('order.comment_msg2')"></textarea>
			</view>
			<view class=""></view>
		...
		<view class="operate margin-bj padding-bj bg-white radius-bock" v-if="shop.shopname">
			...
			<view class="score">
				<view class="title">{{$t('order.comment_msg3')}}</view>
			...
			<view class="score">
				<view class="title">{{$t('order.comment_msg4')}}</view>
			...
			<view class="score">
				<view class="title">{{$t('order.comment_msg5')}}</view>
			...
			<view class="score">
				<view class="title">{{$t('order.comment_msg6')}}</view>
		...
		<view class="wanlian cu-bar tabbar foot flex flex-direction"><button @tap="addData()" class="cu-btn wanl-bg-orange lg">{{$t('order.comment_msg7')}}</button></view>
	</view>
</template>
...
export default {
	...
	onLoad(option) {
		uni.setNavigationBarTitle({
			title:this.$t('order.comment_title')
		})
	...
	methods: {
		...
		async addData() {
			...
			if (this.shop.describe == 0 || this.shop.logistics == 0 || this.shop.service == 0 || this.shop.deliver == 0) {
				this.$wanlshop.msg(this.$t('order.comment_msg8'));
				return false;
			}
			let data = {
		...
		scoreInfo(index){
			if(index == 1){
				return this.$t('order.comment_msg9');
			}else if(index == 2){
				return this.$t('order.comment_msg10');
			}else if(index == 3){
				return this.$t('order.comment_msg11');
			}else if(index == 4){
				return this.$t('order.comment_msg12');
			}else if(index == 5){
				return this.$t('order.comment_msg13');
			}
		}
	}
};
