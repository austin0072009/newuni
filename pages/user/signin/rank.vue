		...
		<view class="cu-list menu-avatar">
			<view class="cu-item" v-for="(item, index) in ranklist" :key="item.id">
				...
				<view class="content">
					...
					<view class="wanl-gray text-sm">{{$t('index.points')}}：{{item.user.score}}<text class="margin-left">{{$t('index.sign_in')}}：{{item.days}}{{$t('signin.day')}}</text></view>
				</view>
				<view class="action">
	...
	export default {
		...
		onLoad() {
			uni.setNavigationBarTitle({
				title:this.$t('signin.rank_title')
			})
			this.loadData();
		},
		methods: {
