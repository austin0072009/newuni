	...
	<view>
		...
		<view class="wanl-goods-comment">
			<view class="padding-bj bg-white margin-bottom-bj head">
				...
				<!-- 标签 -->
				<view>
					<view @tap="onTag('')" class="cu-tag round" :class="{ active: tag == '' }">{{$t('groups.all')}} ({{ statisticsData.total }})</view>
					<view @tap="onTag('good')" class="cu-tag round" :class="{ active: tag == 'good' }">{{$t('good.msg20')}} ({{ statisticsData.good }})</view>
					<view @tap="onTag('pertinent')" class="cu-tag round" :class="{ active: tag == 'pertinent' }">{{$t('good.msg21')}} ({{ statisticsData.pertinent }})</view>
					<view @tap="onTag('poor')" class="cu-tag round" :class="{ active: tag == 'poor' }">{{$t('good.msg22')}} ({{ statisticsData.poor }})</view>
					<view @tap="onTag('figure')" class="cu-tag round" :class="{ active: tag == 'figure' }">{{$t('good.msg23')}} ({{ statisticsData.figure }})</view>
			...
			<view class="list padding-bj radius-bock" v-for="item in listData" :key="item.id">
				...
				<view class="details">
					...
					<view class="text-gray text-sm">{{$t('good.msg24')}}：{{ item.suk }}</view>
				</view>
...
export default {
	...
	onLoad(option) {
		uni.setNavigationBarTitle({
			title:this.$t('good.comment_title')
		})
