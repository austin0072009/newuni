<template>
	<view>
		...
		<view class="wanl-order"> 
			...
			<!-- <view class="cu-list menu-avatar margin-bj radius-bock">
				<view class="cu-item">
					参与user_id拼团
				</view>
			</view> -->
			<view v-for="(shop, index) in orderData.lists" :key="index" class="lists bg-white margin-bj radius-bock">
				...
				<view class="cu-list menu-avatar">
					<view class="cu-item margin-bottom" v-for="(goods, key) in shop.products" :key="key">
						...
						<view class="action">
							<block v-if="goods.groupType == 'ladder'">
								<view class="wanl-pip"> {{goods.ladder.people_num}}人 阶梯价</view>
								<view class="wanl-pip text-sm text-price">
									{{ goods.sku.price *  goods.ladder.discount / 100}}
								</view>
							</block>
							<block v-else-if="goods.groupType == 'group'">
								<view class="wanl-pip"> 拼团价 </view>
								<view class="wanl-pip text-sm text-price">{{ goods.sku.price }}</view>
							</block>
							<block v-else-if="goods.groupType == 'alone'">
								<view class="wanl-pip"> 单购价 </view>
								<view class="wanl-pip text-sm text-price">{{ goods.sku.market_price }}</view>
							</block>
				...
				<form>
					...
					<!-- <view class="cu-form-group">
						<view class="wanl-gray-light title">团长优惠</view>
						<view class="picker text-price margin-left-xs"></view>
					</view> -->
...
export default {
	...
	methods: {
		async loadData(address_id) {
			this.$api.post({
				url: '/wanlshop/groups/order/getOrderGoodsList',
				loadingTip: '加载中',
				data: {
		...
		async addOrder() {
			...
			this.orderData.lists.forEach((item, index)=> {
				...
				item.products.forEach(goods => {
					...
					data.lists[index].products.push({
						...
						ladder_id: goods.ladder ? goods.ladder.id : 0,
						groupType: goods.groupType,
						groups_id: goods.groups_id
					});
				});
			});
			this.loading = true;
			// 提交订单数据data
			this.$api.post({
				url: '/wanlshop/groups/order/addOrder',
				data: {
					order: data,
					token: this.token
				},
				...
				success: res => {
					this.$store.commit('statistics/order', {
						groups: this.$store.state.statistics.order.groups + cart.length
					});
					// 如果使用优惠券全局减去
