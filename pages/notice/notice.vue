<template>
	<view class="notice">
		...
		<view class="wanl-notice bg-bgcolor">
			<view class="tool">
				<view class="text-sm margin-right" v-if="statistics.notice.chat > 0">{{statistics.notice.chat}}{{$t('notice.msg1')}}</view>
				<view class="text-sm margin-right" v-else>{{$t('notice.msg2')}}</view>
				<!-- #ifdef MP -->
				<view class="text-sm" @tap="empty()"><view class="cu-tag round sm margin-right-xs wlIcon-qingkong"></view>{{$t('notice.msg3')}}</view>
				<!-- #endif -->
			</view>
		...
		<view class="wanl-msg" v-if="chat.list.length != 0">
			<view class="cu-list menu-avatar">
				<view class="cu-item" v-for="(item, index) in chat.list" :key="index" @tap="toChat(item.id)" :class="modalName=='move-box-'+ item.id?'move-cur':''" @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + item.id">
					...
					<view class="move">
						<view class="bg-red" @tap.stop="del(index)">{{$t('notice.msg7')}}</view>
					</view>
				</view>
			</view>
		</view>
		
		<wanl-empty src="notice_default3x" :text="$t('notice.empty')" v-else/>
