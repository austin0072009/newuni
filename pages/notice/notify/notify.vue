			...
			<view class="item margin-lr-bj margin-tb bg-white radius-bock padding-bj" @tap="$wanlshop.to(item.url)">
				...
				<view class="action">
					...
					<view class="padding-xs flex flex-wrap align-content-between">
						...
						<text class="text-sm wanl-gray">{{$t('notice.msg8')}} {{item.come}}</text>
					</view>
				</view>
			</view>
			</block>
		</view>
		<wanl-empty src="notice_default3x" :text="$t('notice.notify_empty')" v-else/>
	...
	export default {
		...
		onLoad() {
			uni.setNavigationBarTitle({
				title:this.$t('notice.notify_title')
			})
			this.loadData();
			// 清除本地角标
