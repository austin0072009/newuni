...
<template>
	<view>
		...
		<wanl-empty v-if="addressList.length === 0 && status !== 'loading'" :text="$t('address.empty')"/>
		<view class="margin-top-bj">
			<view class="cu-list menu-avatar">
				<view class="cu-item" :class="modalName == 'move-box-' + index ? 'move-cur' : ''" v-if="addressList" v-for="(item, index) in addressList" :key="item.id" @tap="checkAddress(item)" @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + index" >
					...
					<view class="move">
						<view class="bg-grey" @tap.stop="addAddress('edit', item)">{{$t('address.edit')}}</view>
						<view class="bg-red" @tap.stop="delAddress(item.id, index)">{{$t('cart.delete')}}</view>
					</view>
				</view>
		...
		<view class="wanlian cu-bar tabbar foot flex flex-direction"><button @tap="addAddress('add')" class="cu-btn wanl-bg-orange lg">+ {{$t('address.add')}}</button></view>
	</view>
</template>
