	...
	<view class="wanl-goods">
				...
				<view class="wan-live" v-if="goodsData.isLive" @tap="onLive(goodsData.isLive.id)">
					<view class="tag shadow-blur" :style="{top:wanlsys.height + 47 + 'px' }">
						<view class="text-center">
							...
							<text class="text-white margin-lr-xs"> {{$t('good.msg1')}} </text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="bg-white">
				...
				<view class="price align-center  padding-lr-bj">
					<view>
						<view class="text-price text-white text-xxl text-bold margin-right-xs">0<text class="text-lg">.00</text>
							<text class="margin-left-sm text-sm">{{$t('good.msg2')}}<text class="amount">0</text>{{$t('index.a')}}</text>
						</view>
						<view class="text-sm text-white text-price"><text class="text-dec">0.00</text></view>
					</view>
					<view>
						<view class="text-xs margin-bottom-xs">{{$t('good.msg3')}}：</view>
						<wanl-countdown 
							:time="1" 
							:height="40" 
							:width="40" 
			...
			<view v-else class="price margin-bottom-sm padding-lr-bj padding-top-bj align-start">
				...
				<view class="follow wanl-black" v-if="goodsData.follow" @tap="follow">
					...
					<text class="text-xs">{{$t('good.msg5')}}</text>
				</view>
				<view class="follow wanl-black" v-else @tap="follow">
					<text class="wlIcon-gerenguanzhu"></text>
					<text class="text-xs">{{$t('good.msg6')}}</text>
				</view>
			</view>
			<!-- 标题，分享 -->
			<view class="title padding-lr-bj padding-bottom-bj">
				<view class="name wanl-black text-lg text-cut-2">
					<view v-if="goodsData.shop.isself == 1" class="cu-tag radius margin-right-xs sm bg-red">{{$t('index.proprietary')}}</view>
					{{ goodsData.title || $t('good.msg7') }}
				</view>
				<view class="share" @tap="showModal('share')">
					<view class="button wanl-gray-light margin-bottom">
						...
						<text class="text-sm">{{$t('gg.msg10')}}</text>
					</view>
				</view>
			</view>
		...
		<view class="promotion text-sm">
			...
			<view class="item" @tap="showModal('coupon')" v-if="goodsData.coupon.length != 0">
				<view class="label wanl-gray">{{$t('good.msg10')}}</view>
				...
				<view class="bnt-quan wanl-gray">
					{{$t('good.msg11')}}<text class="wlIcon-fanhui2 margin-left-xs"></text>
				</view>
			</view>
		</view>
		<!-- 选择商品 -->
		<view class="choice margin-bottom-bj padding-bj text-sm bg-white">
			<view class="commodity  margin-bottom-bj" @tap="showModal('option')">
				<view class="opt">
					<view class="title wanl-gray">{{$t('good.msg12')}}</view>
					<view class="option">
						<view class="selected">
							<view v-if="canCount">
								{{$t('good.msg12')}}
						...
						<view class="option-list">
							<view v-for="(item, index) in goodsData.spu" :key="item.id">
								...
								<text class="cu-tag" v-if="index == 0">{{$t('good.msg14')}}</text>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="commodity" @tap="showModal('service')">
				<view class="opt">
					<view class="title wanl-gray text-sm">{{$t('good.msg15')}}</view>
					<view class="option">
						<view class="selected">
		...
		<view class="choice margin-bottom-bj padding-bj text-sm bg-white">
			<view class="commodity" @tap="showModal('attribute')">
				<view class="opt">
					<view class="title wanl-gray text-sm">{{$t('good.msg16')}}</view>
					<view class="option">
						<view class="selected"><text>{{$t('good.msg17')}}</text></view>
					</view>
				</view>
				<view class="text-min wanl-gray"><text class="wlIcon-fanhui2"></text></view>
			</view>
		...
		<view class="shop padding-bj solid-bottom bg-white">
			<view class="shopinfo margin-bottom">
				...
				<view class="shopname">
					...
					<view class="wanl-gray text-min">{{ $wanlshop.toFormat(goodsData.shop.find_user.fans, 'thousand') }} {{$t('good.msg26')}}</view>
				</view>
				<view class="bnt">
					<!-- 1.0.9升级 -->
			...
			<!-- <view class="quality wanl-gray text-min">
				<view>
					...
					<view v-if="goodsData.shop.score_describe < 4.7" class="cu-tag round di">{{$t('good.msg30')}}</view>
					<view v-else class="cu-tag round gao">{{$t('good.msg31')}}</view>
				</view>
				<view>
					{{$t('good.msg32')}}
					<text class="wanl-orange">{{ goodsData.shop.score_service }}</text>
					<view v-if="goodsData.shop.score_service < 4.7" class="cu-tag round di">{{$t('good.msg30')}}</view>
					<view v-else class="cu-tag round gao">{{$t('good.msg31')}}</view>
				</view>
				<view>
					{{$t('good.msg33')}}
		...
		<view class="WANL-MODAL text-sm" @touchmove.stop.prevent="moveHandle">
				...
				<view class="cu-dialog" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">{{$t('good.msg37')}}</view></view>
								...
								<view class="item-right padding-bj">
									...
									<view class="content text-min">
										...
										<view class="cu-btn sm round" @tap="onReceive(index)"  v-if="!coupon.state">
											<text>{{$t('coupon.immediately_to_receive')}}</text>
										</view>
									</view>
								</view>
							</view>
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'attribute' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">{{$t('good.msg39')}}</view></view>
						</view>
						<scroll-view class="scroll-y" scroll-y="true">
							<view class="table solid-bottom">
								<view class="name wanl-gray">{{$t('product.msg7')}}</view>
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'service' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">{{$t('good.msg40')}}</view></view>
						</view>
					...
					<view class="option wanl-modal">
						<view class="head text-left padding-bj solid-bottom">
							...
							<view>
								...
								<view v-if="selectArr.join('')" class="wanl-gray margin-bottom-xs">
									{{$t('good.msg41')}} <text class="margin-lr-xs">{{ selectshop.stock || 0 }}</text> {{$t('index.a')}}
								</view>
								<view class="text-sm">
									<view v-if="selectArr.join('')">{{$t('good.msg42')}}：{{ selectArr.join(' ') }}</view>
									<view v-else>
										{{$t('good.msg43')}}：
						...
						<scroll-view scroll-y="true" :style="{maxHeight: wanlsys.screenHeight / 2 + 'px'}">
							...
							<view class="number padding-bj">
								<view class="text-df">{{$t('good.msg44')}}</view>
						...
						<view class="foot padding-lr-bj">
							<button v-if="isChoice" class="cu-btn bg-gradual-orange round text-bold complete" @tap="completeSelection">{{$t('gg.msg1')}}</button>
							<block v-else>
								<button class="cu-btn bg-gradual-yellow round-left text-bold" @tap="SubmitData('addToCart', 1)">{{$t('good.msg45')}}</button>
								<button class="cu-btn bg-gradual-orange round-right text-bold" @tap="SubmitData('placeOrder', 1)">{{$t('good.msg46')}}</button>
							</block>
						</view>
					</view>
				</view>
		...
		<view class="wanlian cu-bar tabbar shop solid-top foot">
			...
			<view class="btn-group">
				<button class="cu-btn bg-gradual-yellow round-left" @tap="SubmitData('addToCart')">{{$t('good.msg45')}}</button>
				<button class="cu-btn bg-gradual-orange round-right" @tap="SubmitData('placeOrder')">{{$t('good.msg48')}}</button>
			</view>
		</view>
	</view>
</template>
...
export default {
	data() {
		return {
			...
			anchorlist: [{ name: this.$t('good.msg49'), top: 0 }, { name: this.$t('index.evaluation'), top: 0 }, { name: this.$t('good.msg50'), top: 0 }, { name: this.$t('index.recommended'), top: 0 }], //导航条锚点
			selectAnchor: 0,
			//产品数据
			goodsData: {
				id: 0,
	...
	methods: {
		...
		changeNum(val) {
			if(this.selectshop.stock){
				...
				if(num > this.selectshop.stock){
					this.$wanlshop.msg(this.$t('good.msg52') + this.selectshop.stock + this.$t('index.a'));
		...
		completeSelection() {
			if (this.canCount) {
				this.$wanlshop.msg(this.$t('good.msg53'));
			} else {
				this.ProcessingData();
			}
		},
		...
		ProcessingData() {
			if (this.isChoice == 'addToCart') {
				...
				this.$wanlshop.msg(this.$t('good.msg54'));
				...
				this.$wanlshop.msg(this.$t('good.msg55'));
			}
			this.hideModal();
		},
		productSearch(text) {
